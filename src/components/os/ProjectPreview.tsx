...
/**
 * Generative preview artwork per project.
 * Each variant draws an abstract, stylized UI
          ...
          <div className="grid grid-cols-3 gap-1">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                ...
                style={{
                  backgroundColor: i % 2 ? `${accent}30` : `${accent2}25`,
                }}
              />
            ))}
          </div>
...
/* =========================================================
   EKONEXA / DASHBOARD PREVIEW
========================================================= */

function DashboardArt({ accent, accent2, animated }: ArtProps) {
  const chartData = [35, 55, 42, 68, 50, 78, 62, 88, 72];
  ...
  return (
    <div className="flex h-full flex-col gap-2">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div
          className="rounded-md border border-white/[0.06] p-2"
          style={{
      ...
      <div className="flex flex-1 gap-2">
        {/* Sidebar */}
        <div className="w-1/5 rounded-md border border-white/[0.06] p-1.5">
        ...
        <div className="flex flex-1 flex-col gap-2">
          <div className="flex flex-1 items-end gap-1 rounded-md border border-white/[0.06] p-2">
            {chartData.map((height, i) => (
              <motion.div
                ...
                style={{
                  background: `linear-gradient(to top, ${accent}, ${accent2}88)`,
                }}
              />
            ))}
          </div>
          ...
          <div className="flex gap-1.5">
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="h-5 flex-1 rounded-md border border-white/[0.06]"
                style={{
                  background: `${accent2}10`,
                }}
              />
...
function LearningArt({ accent, accent2 }: ArtProps) {
  return (
    <div className="flex h-full gap-2.5">
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center gap-1">
            <div
              ...
              style={{
                backgroundColor: i < 3 ? accent : "rgba(255,255,255,0.12)",
              }}
            />
            ...
            <div
              ...
              style={{
                backgroundColor:
                  i < 3 ? `${accent}50` : "rgba(255,255,255,0.07)",
              }}
            />
          </div>
        ))}
      ...
      <div className="flex flex-1 flex-col gap-2">
        <div
          className="relative flex-1 overflow-hidden rounded-md border border-white/[0.06]"
          style={{
            background: `linear-gradient(120deg, ${accent2}18, transparent)`,
          }}
        >
          <div
          ...
          <div
            className="absolute bottom-2 left-2 right-2 h-1 rounded"
            style={{
        ...
        <div className="flex gap-1.5">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-6 flex-1 rounded-md border border-white/[0.06]"
              style={{
  ...
  return (
    <div className="relative flex h-full items-center justify-center overflow-hidden">
      ...
      <motion.div
        ...
        style={{
          background: `linear-gradient(135deg, ${accent2}, transparent)`,
        }}
      />

      <div className="z-10 text-center">
        <p className="font-display text-xl font-black tracking-tight text-white/90">
          W
        </p>

        <div
          className="mx-auto mt-1.5 h-1 w-10 rounded"
          style={{
